...
public class Contorl {
    ...
    public void run(){
        int number;
        while(menu_layer!=-1){        // 当 menu_layer 的值等于 -1 时退出程序
            ...
            }else if(menu_layer==1){                 //  ----------------> 跳转到查询功能菜单
                operationQuery();
            } else if(menu_layer==2){             // -------------> 跳转到添加功能菜单
                operationAdd();
            }else if(menu_layer==3){                  // --------->  跳转到删除功能菜单
                operationRemove();
            }else if(menu_layer==4){               // -----------> 跳转到修改功能菜单
                operationAlert();
            }
            System.out.println(BLANK);
        }
